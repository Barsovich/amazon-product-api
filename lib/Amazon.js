...
class AmazonScraper {
    ...
    grabProduct(body, p) {
        ...
        for (let key in scrapingResult) {
            ...
            try {
                let url = $(`div[data-index][data-asin=${key}] [href*="/dp/"]:not([href*="#"])`)[0].attribs.href;
                if (!url.startsWith(this.mainHost)) {
                    url = this.mainHost + url;
                }
                console.log({ url, key });
                console.log('First check', url.startsWith(this.mainHost));
                console.log('Second check', url.contains(`/dp/${key}`));
                if (url.startsWith(this.mainHost) && url.contains(`/dp/${key}`)) {
                    console.log('Setting url');
                    scrapingResult[key].url = url;
                }
            } catch (err) {
                continue;
            }
